...
export async function POST(req: NextRequest) {
  ...
  console.log(channelResourceId," -> channel resource id");
      ...
      if (workflow) {
        workflow.map(async (flow) => {
          ...
          while (current < flowPath.length) {
            // console.log(flowPath[current],"- - -");

            if (flowPath[current] == 'Discord') {
              const discordMessage:any = await db.discordWebhook.findFirst({
                where: {
                  userId: flow.userId,
                },
                select: {
            ...
            if (current===1) { // (flowPath[current]=='Slack') - was not working
              // console.log("slack");
